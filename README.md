...
종목 카드의 **백분위 1y/3y**는 최신 일별 종가 괴리율 이하인 표본의 비율입니다.
holding_value와 동일하게 최근 365일/1,095일, 최소 유효 표본 30개를 기준으로 정수 %를 표시합니다.
`data/summary.json`의 `spreadPercentiles`에 기준일과 값을 저장하며 장중에는 재계산하지 않습니다.
표본이 부족한 기간은 `-`, 두 기간 모두 부족하면 행을 생략합니다.
