...
- `js/` — 대시보드 로직 ES 모듈. 빌드 도구 없이 GitHub Pages가 그대로 서빙합니다.
  - `main.js`(진입점·부트스트랩) / `state.js`(공유 상태 `app`·상수·데이터 로더) / `format.js`(포맷터·날짜 유틸) / `calc.js`(지수·통계 계산) / `market.js`(시장 메트릭 병합·세션 로직) / `labels.js`(종목 라벨/배지) / `stale.js`(거래정지·데이터 정체 판정과 기준일 배지) / `charts.js`(캔버스 차트·줌) / `views.js`(렌더·바인딩) / `live.js`(실시간 갱신)
...
| 파일 | 내용 |
|---|---|
| `data/summary.json` | 전 종목 메타데이터 + 현재가 스냅샷 + 종목별 마지막 히스토리 날짜(`lastHistoryDate`) (대시보드 최초 로드용) |
...
- **종목 단위 격리(1종목 장애가 전체를 멈추지 않게)**: 한 종목의 신규 데이터가 비어도 그 종목만 직전 기록으로 유지하고(`carry_forward_missing_pairs`) 나머지 종목의 갱신은 계속합니다. 예전에는 결과에서 빠진 종목을 품질 가드가 위반으로 판정해 exit 1 → 60개 전 종목의 히스토리가 통째로 멈췄습니다. 실제 사례: 2026-07-30 한화(`000880`/`00088K`)가 거래정지되어 거래량 0·종가 고정 데이터만 나오자 `거래일 없음` 스킵 → 가드 위반 → 2026-08-07부터 15일간 일별 워크플로우가 매일 실패했습니다. (2026-08-14부터는 대교 `019680`도 같은 상태입니다.)
...
- **정체 종목 표시(대시보드)**: 거래정지 종목은 직전 기록이 유지되므로 `pair.current`가 몇 주 전 가격입니다. 이를 오늘 가격으로 오독하지 않도록, `data_writer.py`가 종목별 마지막 히스토리 날짜를 `summary.json`의 `pair.lastHistoryDate`로 노출하고, 프런트(`js/stale.js`)가 전체 최신일보다 5일 넘게 뒤처진 종목의 카드·테이블에 `2026.07.29 기준` 배지를 붙입니다. 실시간 시세가 히스토리에 반영돼도 기준일이 밀리지 않도록 로드 시점의 파이프라인 날짜를 고정해 씁니다. 파이프라인 경고(14일)보다 촘촘한 것은 목적이 달라서입니다 — 경고는 상장폐지 확인용, 배지는 "지금 보는 값이 언제 값인지" 표시용입니다.
